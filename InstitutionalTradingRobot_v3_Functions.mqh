...
void AnalyzeOrderBlockInteractions()
{
    ...
    for(int i = 0; i < ob_count; i++)
    {
        ...
        if(price_in_ob && !prev_in_ob)
        {
            ...
            {
                if(is_heading)
                    AddPriceActionComment("→ Price HEADING to bullish OB [" + IntegerToString(order_blocks[i].id) + "] from below (1st time) - IDEAL setup for longs",
                                         clrLime, PRIORITY_IMPORTANT);
                else
                    AddPriceActionComment("→ Price REVISITING bullish OB [" + IntegerToString(order_blocks[i].id) + "] from below - Zone still holding, good for longs",
                                         clrLime, PRIORITY_IMPORTANT);
            }
            else if(order_blocks[i].is_bullish && entering_from_top)
            {
                if(is_heading)
                    AddPriceActionComment("→ Price HEADING to bullish OB [" + IntegerToString(order_blocks[i].id) + "] from above (1st time) - Possible retest or reversal",
                                         clrYellow, PRIORITY_IMPORTANT);
                else
                    AddPriceActionComment("→ Price REVISITING bullish OB [" + IntegerToString(order_blocks[i].id) + "] from above - Re-test in progress, zone weakening",
                                         clrOrange, PRIORITY_IMPORTANT);
            }
            else if(!order_blocks[i].is_bullish && entering_from_top)
            {
                if(is_heading)
                    AddPriceActionComment("→ Price HEADING to bearish OB [" + IntegerToString(order_blocks[i].id) + "] from above (1st time) - IDEAL setup for shorts",
                                         clrRed, PRIORITY_IMPORTANT);
                else
                    AddPriceActionComment("→ Price REVISITING bearish OB [" + IntegerToString(order_blocks[i].id) + "] from above - Zone still holding, good for shorts",
                                         clrRed, PRIORITY_IMPORTANT);
            }
            else if(!order_blocks[i].is_bullish && entering_from_bottom)
            {
                if(is_heading)
                    AddPriceActionComment("→ Price HEADING to bearish OB [" + IntegerToString(order_blocks[i].id) + "] from below (1st time) - Possible retest or reversal",
                                         clrYellow, PRIORITY_IMPORTANT);
                else
                    AddPriceActionComment("→ Price REVISITING bearish OB [" + IntegerToString(order_blocks[i].id) + "] from below - Re-test in progress, zone weakening",
                                         clrOrange, PRIORITY_IMPORTANT);
            }

            AddPriceActionComment("→ OB [" + IntegerToString(order_blocks[i].id) + "] Test #" + IntegerToString(order_blocks[i].test_count + 1) +
            ...
            {
                AddPriceActionComment("⚠ WARNING: OB [" + IntegerToString(order_blocks[i].id) + "] Final test - Next break invalidates it",
                                     clrOrange, PRIORITY_IMPORTANT);
            }
        }
        ...
        if(prev_in_ob && !price_in_ob)
        {
            ...
            if(rejection)
            {
                AddPriceActionComment("✓ STRONG REJECTION from " + ob_type + " ORDER BLOCK [" + IntegerToString(order_blocks[i].id) + "]!",
            ...
            {
                AddPriceActionComment("⛔ " + ob_type + " ORDER BLOCK [" + IntegerToString(order_blocks[i].id) + "] BREACHED - Invalidated",
        ...
        {
            string ob_date = FormatTimeDifference(order_blocks[i].time);
            AddPriceActionComment("📍 Price INSIDE " + ob_type + " ORDER BLOCK [" + IntegerToString(order_blocks[i].id) + "] - Decision zone [" + ob_date + "]",
                                 clrCyan, PRIORITY_INFO);
